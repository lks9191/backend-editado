...
public class DatabaseInitializer implements CommandLineRunner {
    ...
    @Autowired
    private UsuarioRepository usuarioRegistroRepository;
    ...
    private void inicializarRoles() {
        if (rolRepository.count() == 0) {
            ...
            rolSeguridad.setNombre(Rol.NombreRol.ROL_SEGURIDAD);
            rolRepository.save(rolSeguridad);

            Rol rolMantenimiento = new Rol();
            rolMantenimiento.setNombre(Rol.NombreRol.ROL_MANTENIMIENTO);
            rolRepository.save(rolMantenimiento);
    ...
    private void crearAdminPorDefecto() {
        if (!usuarioRegistroRepository.existsByUsername("admin")) {
            ...
            usuarioRegistroRepository.save(admin);

            System.out.println("UsuarioRegistro administrador creado: admin / admin123");
        }
    }
}