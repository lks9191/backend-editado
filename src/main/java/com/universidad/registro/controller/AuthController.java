...
import com.universidad.registro.dto.AuthDTO;
...
public class AuthController {
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody AuthDTO.SignupRequest signUpRequest) {
        if (usuarioRepository.existsByUsername(signUpRequest.getUsername())) {
            return ResponseEntity
                    .badRequest()
                    .body(new AuthDTO.MessageResponse("Error: El nombre de usuario ya está en uso."));
        }
        ...
        if (usuarioRepository.existsByEmail(signUpRequest.getEmail())) {
            return ResponseEntity
                    .badRequest()
                    .body(new AuthDTO.MessageResponse("Error: El email ya está en uso."));
        }
        ...
        if (strRoles == null) {
            Rol usuarioRol = rolRepository.findByNombre(Rol.NombreRol.ROL_ESTUDIANTE)
            ...
            strRoles.forEach(role -> {
                switch (role) {
                    case "admin":
                        ...
                        break;
                    case "empleado":
                        Rol empleadoRol = rolRepository.findByNombre(Rol.NombreRol.ROL_DOCENTE)
                                .orElseThrow(() -> new RuntimeException("Error: Rol no encontrado."));
                        roles.add(empleadoRol);
                        break;
                    default:
                        Rol usuarioRol = rolRepository.findByNombre(Rol.NombreRol.ROL_SEGURIDAD)
                                .orElseThrow(() -> new RuntimeException("Error: Rol no encontrado."));
                        roles.add(usuarioRol);
                }
            });
        ...
        return ResponseEntity.ok(new AuthDTO.MessageResponse("Usuario registrado exitosamente!"));
    }
    ...
    public ResponseEntity<?> logoutUser() {
        ...
        return ResponseEntity.ok(new AuthDTO.MessageResponse("Sesión cerrada exitosamente!"));
    }
}
